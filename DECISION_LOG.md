...
### Layer-2 CoT diagnostic on Phase D residual FPs (added 2026-04-30 14:48)

Ran CoT on 50 of NBA's 119 intervention FPs and 50 of CSGO's 358 intervention FPs (`run_phase_d_cot.py` → `RESULTS/phase_d_cot_residual_fps.json`). For each FP the model was asked to quote the rule it thinks was violated and name the event index.

**NBA**: 46/50 (92%) cite "Offensive team must shoot within 24 seconds of gaining possession"; 4/50 (8%) cite the foul ejection rule. The shot-clock cluster is a *second-order* failure mode — D-44's `possession_elapsed_s → time_in_possession_s` rename eliminated the first-order misreading of the per-event timing field, but at n=1,200 the model still infers shot-clock violations from chain context structure ~10% of the time. Renderer fix (deferred to v5.2): surface explicit `POSSESSION_START_AT=t_n` markers.

**CSGO**: 40/50 (80%) cite "Bomb plants only at sites A or B"; 10/50 (20%) cite the respawn-timing rule. Both are textbook constraint-triggered confabulation — the model knows the rules, the FACEIT aggregate data doesn't surface the predicate variables (no plant events with `site=A/B`, no per-round respawn info), and the model concludes the violations must have happened off-chain. Confirms CSGO is observability-bound, not reasoning-bound. Awpy fix (deferred to v5.2) is the clean test of this hypothesis.
